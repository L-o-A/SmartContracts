...
async function main() {
   ...
   //  await multiSigAdmin.deployed();
    const MultiSigAdmin = await ethers.getContractFactory("MultiSigAdmin");
    const multiSigAdmin = await MultiSigAdmin.attach("0xA8eB353d0B074d2D81A986c694e771D29e617F2f");
   ...
   const RaffleHelper = await ethers.getContractFactory("RaffleHelper");
   const raffleHelper = await RaffleHelper.attach("0xD35902Bb1acAb731c14048569cecd4441c046714");
   ...
   //  await raffle.deployed();
    const Raffle = await ethers.getContractFactory("Raffle");
    const raffle = await Raffle.attach("0x5C2E898bbBf6dC00d6bC72710B7d8232a8b8b18a");
   ...
   const Capsule = await ethers.getContractFactory("Capsule");
   const capsule = await Capsule.attach("0x46800976e8738392418e2D7F32A7A25BcdD6F80f");
   ...
   const CapsuleStaking = await ethers.getContractFactory("CapsuleStaking");
   const capsuleStaking = await CapsuleStaking.attach("0xDBaa0254c173E42B2568C282aa0F58E56036a2e7");
   ...
   const LoANFT = await ethers.getContractFactory("LoANFT");
   const _LoANFT = await LoANFT.attach("0xD04B35436d122DB3662Fe07292938477B0a6a48c");
   ...
   const LoANFTAttributes = await ethers.getContractFactory("LoANFTAttributes");
   const _LoANFTAttributes = await LoANFTAttributes.attach("0xB2984fbf12D6D96CE44E9A7558A40866a6edfBB0");
   ...
   const LoANFTFusion = await ethers.getContractFactory("LoANFTFusion");
   const _LoANFTFusion = await LoANFTFusion.attach("0x7E1223Fd54dB975d53249DDcaB196F8D630cf272");
   ...
   const NFTMarket = await ethers.getContractFactory("NFTMarket");
   const _NFTMarket = await NFTMarket.attach("0xeB00CfE627c98970b8b0c995DFD8Af993F2B9D78");
   ...
   //  console.log(await raffle.getUserTickets(treasury));
    ...
    await capsule.setApprovalForAll(capsuleStaking.address, true);
    console.log(19.1);
    
    await _loa.approve(capsuleStaking.address, "10000000000000000000000000")
   //  console.log(await _loa.allowance(treasury, capsuleStaking.address));
    console.log(await capsuleStaking._capsuleToken());
    console.log(19.2);
   //  await capsuleStaking.stake([6]);
    console.log(await capsule.balanceOf(treasury,  6));
    console.log(20);
   ...
   //  await capsuleStaking.reclaim([6], false);
    console.log(21);
    console.log(await capsule._capsule_status(6));

    
    await capsule.setApprovalForAll(_LoANFT.address, true);
    ...
    await _loa.approve(_NFTMarket.address, "1000000000000000000000000");
    await _LoANFT.setApprovalForAll(_NFTMarket.address, true);

    console.log(await _NFTMarket.fetchMarketItems());
    const myListings = await _NFTMarket.fetchMyListings();

    console.log("NFT balance", await _LoANFT.balanceOf(treasury, 10));
    
   // await _NFTMarket.list(_LoANFT.address, nfts[0], "200000000000000000000000");
    console.log(24);

    //  await _NFTMarket.unlist(myListings[0][0]);
    await _NFTMarket.buy(myListings[0][0]);
    console.log(25);

    console.log("NFT balance", await _LoANFT.balanceOf(treasury, 10));

     console.log(await _NFTMarket.fetchMarketItems());
    console.log(await _NFTMarket.fetchMyListings());
