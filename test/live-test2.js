...
describe("LIVE MP Test ", function () {
    it("LIVE MP Test", async function () {
        ...
        do {
            ...
            winn_tickets = await raffle.getUserTickets(addr1.address);
            console.log("add1 tickets count", winn_tickets.length);
        } while(winn_tickets.length > 0)
        ...
        await loa.connect(addr1).approve(capsuleStaking.address, "200000000000000000000000000");
        await capsule.connect(addr1).setApprovalForAll(capsuleStaking.address, true);

        console.log("Capsule stake start");

        for(let j = 0; j < Math.ceil(capsules_data.length/100); j++) {
            let data = [];
            for(let i =0; i < 100; i++) {
                if(capsules_data.length > i + j * 100)
                    data.push(capsules_data[i + j * 100]);
            }

            await capsuleStaking.connect(addr1).stake(data);
            await capsuleStaking.connect(addr1).reclaim(data, false);
        }
